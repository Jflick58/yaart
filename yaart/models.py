...
class JobRequirements(BaseModel):
    skills: List[str] = Field(
        description="Required technical skills and competencies, including programming languages, tools, and technologies"
    )
    experience: List[str] = Field(
        description="Required years and types of experience, including specific domains or roles"
    )
    education: List[str] = Field(
        description="Required education qualifications, including degrees, certifications, and specific fields of study"
    )

class JobDescription(BaseModel):
    url: str = Field(
        description="URL of the job posting for reference"
    )
    role: str = Field(
        description="Job title/role as listed in the posting"
    )
    company: str = Field(
    ...
    requirements: JobRequirements = Field(
        description="Structured job requirements including skills, experience, and education"
    )
...
class Education(BaseModel):
    ...
    location: Optional[str] = Field(
        description="Location of the institution (optional)",
        default=None
    )
...
class Experience(BaseModel):
    ...
    company_description: Optional[str] = Field(
        description="Brief description of the company, e.g., 'Fortune 500 Tech Company' or 'Leading AI Startup'",
        default=None
    )
    location: str = Field(
        description="Location of the job (city, state or 'Remote')"
    )
    dates: str = Field(
        description="Employment duration in MM/YYYY - MM/YYYY format or 'Present' for current positions"
    )
...
class Skill(BaseModel):
    category: str = Field(
        description="Skill category/group name (e.g., 'Programming Languages', 'Cloud Technologies')"
    )
    skills: List[str] = Field(
        description="List of specific skills within this category, prioritized based on job requirements"
    )
...
class Publication(BaseModel):
    ...
    title: str = Field(
        description="Title of the published work"
    )
    date: str = Field(
        description="Publication date in 'Month YYYY' format"
    )

class OpenSourceProject(BaseModel):
    name: str = Field(
        description="Name of the open source project or repository"
    )
...
class TailoredResume(BaseModel):
    ...
    title: str = Field(
        description="Professional title aligned with the target role"
    )
    location: str = Field(
        description="Current location (City, State)"
    )
    phone: str = Field(
        description="Contact phone number in format: ##########"
    )
    email: str = Field(
    ...
    summary: str = Field(
        ...
        I would like to leverage my experience in [key skills] to [outcome the role is looking for]
        at [company name and its goal/product type]'"""
    )
    ...
    publications: Optional[List[Publication]] = Field(
        description="Optional list of relevant publications",
        default=None
    )
    open_source: Optional[List[OpenSourceProject]] = Field(
        description="Optional list of relevant open source contributions",
        default=None
    )
...
'''
        ...
        for exp in self.experience:
            company_desc = f" _{exp.company_description} |" if exp.company_description else ""
            md += f"**{exp.title}**  \n"
